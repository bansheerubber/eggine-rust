// taken from https://raw.githubusercontent.com/lo48576/fbx-viewer/develop/src/fbx/v7400/triangulator.rs
		...
		n => {
			...
			let points_2d: Vec<_> = {
				...
				match smallest_direction(&width) {
					Axis::X => points
						.into_iter()
						.map(|v| Vec2::new(v[1], v[2]))
						.collect(),
					Axis::Y => points
						.into_iter()
						.map(|v| Vec2::new(v[0], v[2]))
						.collect(),
					Axis::Z => points
						.into_iter()
...
/// Returns the vector.
fn get_vec(pvs: &PolygonVertices<'_>, pvi: PolygonVertexIndex) -> anyhow::Result<Vec3> {
...
fn bounding_box<'a>(
	points: impl IntoIterator<Item = &'a Vec3>,
) -> Option<(Vec3, Vec3)> {
...
fn smallest_direction(v: &Vec3) -> Axis {
	if v.x < v.y {
		if v.z < v.x {
			Axis::Z
		} else {
